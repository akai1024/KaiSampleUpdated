...
public class UserService {
    ...
    public ErrorCode createUser(RegisterRequest request) {
        ...
        if (KaiUtil.isEmptyStr(user) || !KaiUtil.isAlphanumericCharacters(user) || user.length() < MIN_USER_SIZE || user.length() > MAX_USER_SIZE) {
            return ErrorCode.ILLEGAL_USER;
        }

        if (KaiUtil.isEmptyStr(password) || !KaiUtil.isAlphanumericCharacters(password) || password.length() < MIN_PWD_SIZE || password.length() > MAX_PWD_SIZE) {
